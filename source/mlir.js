...
mlir.TensorLiteralParser = class {
    ...
    parseElement() {
        ...
        if (this._parser.accept('(')) {
            const real = this.parseElement();
            this._parser.expect(',');
            const imag = this.parseElement();
            this._parser.expect(')');
            return { real, imag };
        }
        // Parse a boolean element
        if (this._parser.match('boolean')) {
            return this._parser.expect();
        }
...
mlir.TosaDialect = class extends mlir.Dialect {
    ...
    parseOperation(parser, opName, op) {
        ...
        if (this._binaryOps.has(opName) || this._unaryOps.has(opName) || this._reduceOps.has(opName) || this._poolOps.has(opName) || opName === 'tosa.argmax' || opName === 'tosa.rescale' || opName === 'tosa.apply_scale' || opName === 'tosa.resize') {
            ...
            // Reference: Use FunctionType pattern for : (type, type) -> type
            if (parser.accept(':')) {
                const type = parser.parseType();
                if (type instanceof mlir.FunctionType) {
                    parser.resolveOperands(op.operands, type.inputs);
                    ...
                    // Single type for operands, check for arrow result types
                    for (const operand of op.operands) {
                        if (!operand.type) {
                            operand.type = type;
                        }
...
mlir.AMDGPUDialect = class extends mlir.Dialect {
    ...
    _parseMNKDimensionList(parser, op, args) {
        parser.expect('[');
        const m = parser.parseInteger();
        parser.expect(',');
        const n = parser.parseInteger();
        parser.expect(',');
        const k = parser.parseInteger();
        parser.expect(']');
        if (args && args.length >= 3) {
...
mlir.TestDialect = class extends mlir.Dialect {
    ...
    parseOperation(parser, opName, op) {
        ...
        if (opName === 'test.call_on_device') {
            ...
            if (parser.accept(',')) {
                if (parser.match('%')) {
                    const deviceOperand = parser.parseAttribute();
